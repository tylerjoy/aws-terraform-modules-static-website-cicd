(function ($) {
  function visitCount() {
    // Retrieve the visit count from localStorage, default to 0
    let visits = Number(localStorage.getItem("visitCount")) || 0;

    // Check if the current session has been counted
    const currentSession = sessionStorage.getItem("session");

    // If no session is active, increment the count
    if (!currentSession) {
      visits++;
      localStorage.setItem("visitCount", visits); // Save updated count
      sessionStorage.setItem("session", true); // Mark session as active
    }
  ...
  // Update the visit count on page load
  document.addEventListener("DOMContentLoaded", () => {
    const visitCountElement = document.getElementById("visitCount");
    const count = visitCount(); // Call visitCount function
    visitCountElement.textContent = count; // Update the displayed count
  });
